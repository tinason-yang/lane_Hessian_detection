...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    for kid in range(num_kpts):
        ...
        if not (x_coord % 640 == 0 or y_coord % 640 == 0):
            ...
            cv2.putText(im, str(kid), (int(x_coord), int(y_coord)), font, 1, (0, 0, 255), 2)
    ...
    for sk_id, sk in enumerate(skeleton):
        ...
        if pos2[0] % 640 == 0 or pos2[1] % 640 == 0 or pos2[0] < 0 or pos2[1] < 0:
            continue
        cv2.line(im, pos1, pos2, (int(0), int(0), int(0)), thickness=2)

        # cv2.putText(im, str(sk_id), pos1, font, 1, (0, 0, 255), 2)
        # cv2.putText(im, str(sk_id), pos2, font, 1, (0, 0, 255), 2)
        # print("第%s点的坐标为" % sk_id, pos1)
